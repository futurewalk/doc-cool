...
func initContainer()  {
    container.CoolDocs = map[string]*Annotation{}
}
...
func (p *DocController) analyzeStruct(sp, rp string, flt bool) {
    ...
    p.walk(readPath, func(cnt string) {
...
func (p *DocController) walk(fp string, fc func(cnt string)) {
    f, err := os.Open(fp)
    if err != nil {
        fmt.Printf("Error: %s\n", err)
        return
...
func (p *DocController) dealGoFile(fp, fln string, ant *Annotation) {
    ...
    readFunc := func(cnt string) {
        ...
        if rs && (ant.Body != nil || ant.ProtoBufControl != "") {
            container.CoolDocs[ant.Id] = ant
        }
    }
    p.walk(fp+separator+fln, readFunc)
}
